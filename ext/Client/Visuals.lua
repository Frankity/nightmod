...
function Visuals:OnReadInstance(p_Instance, p_Guid)
...
function Visuals:SetFlashlight(p_Instance, p_Guid)	
...
function Visuals:FixEnvironmentState(p_State)

	self:SetVisuals(p_State)

end

function Visuals:SetVisuals(p_State)
	local s_ColorCorrection = p_State.s_ColorCorrection
	...
	local s_Sky = p_State.s_Sky

	if s_Sky ~= nil then
	    s_Sky.enable = false
	    s_Sky.brightnessScale = 0.03
	    s_Sky.sunSize = 0.004
	    s_Sky.sunScale = 0.03 -- 0.03 last modified
...
function Visuals:SetVisuals2(p_State)
	self:SetVisuals(p_State)
